...
pub async fn block_call(host: &String, options: BlockOptions) -> Result<BlockResponse, ErrCode> {
    let url = format!("{}/api/blocks/{}", host, options.position);
    let req = reqwest::get(url).await;
    get_method::<BlockResponse>(req, &String::from("Error getting block")).await
}

pub async fn valid_after_block_number(url: &String) -> Result<i32, ErrCode> {
